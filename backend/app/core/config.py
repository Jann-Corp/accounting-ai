...
class Settings(BaseSettings):
    ...
    AI_RECOGNITION_PROMPT: str = (
        "你是一个专业的小票/收据识别助手。请分析这张图片中的所有交易记录，提取每一条记录的信息，以JSON数组格式返回。\n"
        "每条记录包含以下字段：\n"
        "- amount: 交易金额（数字，正数）\n"
        ...
        "- confidence: 识别置信度（0.0-1.0之间的浮点数，真实反映识别准确程度）\n\n"
        "输出JSON Demo（必须严格按照此格式返回，不要有额外文字）：\n"
        "[\n"
